...
knitr::opts_chunk$set(echo = TRUE, dpi = 200, fig.widtsh = 5, fig.height = 4)
```
...
```{r}
ggplot(data=iris_df,
       mapping = aes(x=Sepal.Length, y=Sepal.Width, color='blue')) +
  geom_point()
```
...
> Draw **scatter-plot** (`geom_point`) with **x=Petal.Length, y=Petal.Width** usingclassic theme
...
> Draw **histogram** (`geom_histogram`) of **Petal.Length** using *blue* `color` and *lightblue* `fill` andclassic theme
...
> Draw a [horizontal]{.underline} **box plot** (`geom_boxplot`) of **Petal.Width** for each species on the y-axis with different fill. Remove legend (why?)
