...
### Sometimes biological libraries
...
devtools::install_cran('name_of_package')
devtools::install_bioc('name_of_package')
devtools::install_github('user/name_of_package')
...
students_hours = read.csv("papka\\student_lifestyle_dataset.csv", 
                          row.names = 1,
                          # header = F,
                          na.strings = c('','-','dead'),
                          # comment.char = '#',
...
```{r}
str(students_tibble)
students_tibble %>% glimpse(width = 100) 
```
...
```{r}
students_tibble = students_tibble %>% arrange(desc(Extracurricular_Hours_Per_Day))
students_tibble
```
...
```{r}
students_tibble |> pull(Extracurricular_Hours_Per_Day) |>  median(na.rm = TRUE)
students_tibble |> select(Extracurricular_Hours_Per_Day) |> median(na.rm = TRUE)
```
...
```{r}
students_tibble |> select(GPA, Stress_Level) |> filter(GPA < 2.3)
```
...
# students_tibble |> slice_sample(prop = 0.1)
...
```{r}
students_tibble |> 
  arrange(desc(Social_Hours_Per_Day)) |> 
  head(n=10) |> 
...
```{r}
students_tibble |> rename(Stress = Stress_Level)
```
...
students_tibble = students_tibble |>  
  mutate(Stress_Hours = Extracurricular_Hours_Per_Day + Study_Hours_Per_Day,
         Stress_Hours = round(Stress_Hours), 
         .before = GPA) |> 
  mutate(Study = ifelse(Study_Hours_Per_Day > 7, 'High', 'Low'),
         .before = Study_Hours_Per_Day)
  # mutate(Study = case_when(
  #   Study_Hours_Per_Day > 7 ~ 'High',
  #   Study_Hours_Per_Day < 7 & Study_Hours_Per_Day > 5 ~ 'wow',
  #   TRUE ~ 'Low'
  # ))
...
```{r}
students_tibble |> group_by(Study) |> 
  summarise(study_h_sd = sd(Study_Hours_Per_Day))
```
...
```{r}
students_tibble %>% select(Study) |> distinct()
```
...
students_tibble$GPA[is.na(students_tibble$GPA)] = 
  mean(students_tibble$GPA, na.rm = T)
```
...
dir.create("lec_output")
write_csv(students_tibble, "./lec_output/students.csv") # comma-separated

# write.table(students_tibble, "./lec_output/students.tsv")
