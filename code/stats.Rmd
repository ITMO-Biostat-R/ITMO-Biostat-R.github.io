...
-   **p-value** - the probability that, given a [**true null hypothesis**]{.underline}, your observations will result in such or more extreme value of test statistic
...
![](../images/typeserror.png){width="600"}
...
```{r}
plot(x=seq(-5,5,0.01), y=dt(seq(-5,5,0.01), df=29), 'l', lwd=2,
     main = 'T-distribution df=29',
     xlab='t', ylab='density')
abline(v=qt(0.975, df=29), col='red', lwd=2) 
abline(v=qt(0.025, df=29), col='red', lwd=2)
text(x =c(-3,3), y = 0.3, col='red',
     labels = c('alpha = 0.025', 'alpha = 0.975'), cex=1)
...
t.test(height ~ group, data = data_sunlight_plant) # Welch t-test
```

-   **t-value** = -4.6261
-   **p-value** = 2.137e-05 = $2.137 * 10^{-5}$ = $0.00002137$
    -   $2.137 * 10^{-5} < 0.05 \leftrightarrow p.value < \alpha$
    -   We can with great confidence reject the $H_0$
...
**Mann-Witney test** **(also known as** **Wilcoxon** **rank-sum test**)
...
### Testing non-normal before/after data

```{r}
sample_before = c(runif(8, 5, 8), rnorm(5, 3, 10), rchisq(7, 4))
sample_after = sample_before + rnorm(20, 0, 1)

data_before_after = data.frame(group = rep(c("before", "after"), each=20),
...
```{r wilcoxon-paired}
wilcox.test(sample_before, sample_after,  
            paired = TRUE) ## <---
```
...
```{r}
contingency_table <- matrix(c(12, 8, 6, 10),
                   nrow = 2, byrow = T,
                   dimnames = list(
                     "Group" = c("Treatment", "Placebo"),
                     "Outcome" = c("Cured", "Not cured"))
                   )
contingency_table
```
...
```{r}
contingency_table <- matrix( c(1, 3, 4, 2),
                             byrow = T,
                            nrow = 2,
                            dimnames = list(
                              "Knockout" = c("Yes", "No"),
...
plot(x = plant_height_df$Sunlight_hrs_per_day, y=plant_height_df$Plant_height, 
     main = NULL, 
...
```{r}
corrplot(cor(mtcars), method="color",col = COL2('PRGn'),
         addCoef.col = "black", tl.col = "black", 
         cl.cex = 1, number.cex = 0.8)
```
...
```{r}
newdata = data.frame(nutrients=c(5.6, 9.2, 3))
...
-   ***p-values*** are claculated according to [**Fisher distribution**]{.underline}
