...
$Risk_{pl}$ (to be dead in case of Placebo) = $\dfrac{Dead  | Placebo}{Total\ Placebo}$ =
...
Risk ratio ($RR$) = $\dfrac{Risk_{pl}}{Risk_{tr}} =$
...
```{r}
odds_tr = contingency_table_big['Treatment', 'Dead'] / contingency_table_big['Treatment', 'Cured']
odds_tr
```
...
```{r}
odds_pl = contingency_table_big['Placebo', 'Dead'] / contingency_table_big['Placebo', 'Cured']
odds_pl
```

$Odds \ ratio \ (OR) = \dfrac{Odds_{pl}}{Odds_{tr}} =$
...
```{r}
odds_tr_1 = contingency_table_big['Treatment', 'Dead'] / 1000
odds_tr_1

odds_pl_1 = contingency_table_big['Placebo', 'Dead'] / 1010
odds_pl_1

OR_1 = odds_pl_1 / odds_tr_1
OR_1
```
...
```{r}
mean_laughs = data_md |> 
  group_by(Meme_category) |> 
  summarise(mean_laugh = mean(Laugh_level))
...
cor.test(anscombe$x2, anscombe$y2, method = 'spearman')
cor.test(anscombe$x3, anscombe$y3, method = 'spearman')
...
```{r}
cont_table = table(arth_df$Sex, arth_df$Improved)
cont_table_full = addmargins(cont_table)
cont_table_full
```

```{r}
risk_fem = cont_table_full['Female', 'Marked'] / cont_table_full['Female', 'Sum']
risk_fem

risk_male = cont_table_full['Male', 'Marked'] / cont_table_full['Male', 'Sum']
risk_male

RR = risk_fem / risk_male
RR

RD = risk_fem - risk_male
RD
```

```{r}
odds_fem = cont_table_full['Female', 'Marked'] / cont_table_full['Female', 'None']
odds_fem

odds_male = cont_table_full['Male', 'Marked'] / cont_table_full['Male', 'None']
odds_male

OR = odds_fem / odds_male
OR
```
...
```{r}
MD = mean(arth_df |> filter(Sex == 'Male') |>  pull(Age) |>  mean()) -
          mean(arth_df |> filter(Sex == 'Female') |>  pull(Age) |>  mean())
MD 

sexmean = arth_df |> group_by(Sex) |>  summarise(mean_age = mean(Age))
sexmean$mean_age[1] - sexmean$mean_age[2]
```
...
```{r, eval=FALSE}
cor.test(diamonds_small$clarity, diamonds_small$price)
```

```{r}
cor(diamonds_small$price, 
    diamonds_small$cut |>  as.numeric(), 
    method='spearman')
```
